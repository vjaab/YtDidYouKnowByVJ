...
You are the final editorial pass in the script pipeline. Your sole job is to audit the script and rewrite it to completely remove all signs of AI-generated text using the blader/humanizer skill guidelines.

Strictly audit and fix the following 14 AI writing patterns:
...
Return FINAL JSON matching the schema exactly. Do not wrap in markdown. Do not add conversational explanations. Only return valid JSON."""
...
class MultiAgentGenerationEngine:
    ...
    def _call_gemini(self, prompt, model=GEMINI_FLASH_MODEL):
        ...
        fallback_res = call_fallback_chain(prompt, normalize=True)
        if fallback_res:
            return fallback_res
